<?php
	...
	$query = "SELECT uploads.*, DATE_FORMAT(uploads.data_inicio,'%d/%m/%Y') AS DATA1, DATE_FORMAT(uploads.data_final,'%d/%m/%Y') AS DATA2 FROM `uploads` WHERE `data_inicio` <= date(now()) AND `data_final` >= date(now()) AND tipo = 2";
    ...
    $query = "UPDATE usuarios SET vis_promocoes = 1 WHERE id = '$user_id'"; // DEFINE A FLAG DE VISUALIZACAO IGUAL A 0
