<?php
	...
	$query = "SELECT uploads.*, DATE_FORMAT(uploads.data_inicio,'%d/%m/%Y') AS DATA1, DATE_FORMAT(uploads.data_final,'%d/%m/%Y') AS DATA2 FROM uploads WHERE tipo = 1";
    ...
    if(mysqli_num_rows($result) > 0)
    {
    	while($row = mysqli_fetch_assoc($result))
    	{
    		$data .= '<tr>
				...
				<td>'.$row['obs'].'</td>
				<td>
					<i style="text-align:center; cursor:pointer" onclick="detalhesCampanha('.$row['id'].')" class="fa fa-pencil-square-o"></i>
				</td>
				<td>
					<i style="text-align:center; cursor:pointer" onclick="deletaCampanha('.$row['id'].')" class="fa fa-trash-o"></i>
				</td>
				<td>
